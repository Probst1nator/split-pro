...
describe('parseReceiptItemsResponse (line-items mode)', () => {
  ...
  // JSON.parse accepts all of these without throwing.
  it.each([['null'], ['42'], ['"just a string"'], ['[{"amount":"1.00"}]']])(
    'returns an empty item list for the non-object payload %s',
    (payload) => {
      const result = parseReceiptItemsResponse(payload);
      expect(result.items).toEqual([]);
      expect(result.currency).toBe('USD');
    },
  );
...
describe('parseReceiptResponse rejects non-object payloads', () => {
  it.each([['null'], ['42'], ['"just a string"']])('throws for %s', (payload) => {
    expect(() => parseReceiptResponse(payload)).toThrow();
  });
});
...
describe('normalizeAmountString', () => {
  ...
  /* "1.234" is ambiguous (grouped 1234, or exactly 1.234). Number() accepts it, and reading
     it as a thousands group would inflate it 1000x, much worse than being slightly low. */

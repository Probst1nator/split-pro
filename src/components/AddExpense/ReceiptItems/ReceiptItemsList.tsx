...
export const ReceiptItemsList: React.FC = () => {
  ...
  const addSelectedExpenses = useCallback(async () => {
    ...
    /*
     * The shares were validated against the selected total, but every item is split with its
     * own, smaller amount. Proportional types (EQUAL/PERCENTAGE/SHARE) rescale correctly;
     * the absolute ones do not — EXACT requires the shares to sum to the amount and
     * ADJUSTMENT requires them not to exceed it, so reusing them per item yields
     * participant amounts that do not net to zero. calculateParticipantSplit already
     * reports this per call, so validate each item rather than trusting the total.
     */
    ...
    for (const item of selectedItems.filter(
      ...
      /* Drop participants who owe nothing on this item; a rounded-away item has none left. */
  ...
  return (
    <div className="flex flex-col gap-3">
      {/* Sibling of the button, not a child: the synthetic click from inputRef.current.click()
          would otherwise bubble back to the button and re-enter its handler, and an input
          inside a button is invalid content that breaks keyboard and screen-reader behaviour. */}
      <input
        ref={inputRef}
        type="file"
        accept="image/*"

...
export const ReceiptItemsList: React.FC = () => {
  ...
  const handleCapture = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      /* Cleared up front, not on the way out: the File is already held, and an early return on
         failure would leave the input set, so re-picking the same file fires no change event. */
      event.target.value = '';
      ...
      try {
        ...
        if (data.date) {
          setExpenseDate(parseDateOnly(data.date));
        }
  ...
  /* `amountStr` as well as `amount`: the Single tab renders the string, so writing only the
     BigInt left its input empty while Save still submitted this total. */
  useEffect(() => {
    if (receiptItems.length > 0) {
      setAmount(selectedTotal);
      setAmountStr(helpers.toUIString(selectedTotal, false, true));
    }
  }, [selectedTotal, receiptItems.length, setAmount, setAmountStr, helpers]);
  ...
  const addSelectedExpenses = useCallback(async () => {
    ...
    /* Shares were validated against the selected total, but each item splits a smaller amount.
       EQUAL/PERCENTAGE/SHARE rescale; EXACT/ADJUSTMENT do not, so validate per item. */
    const expenses: CreateExpense[] = [];

    for (const item of selectedItems.filter(
      (item) => 0n !== item.amount && '' !== item.description,
    )) {
      const expenseDate = useAddExpenseStore.getState().expenseDate;

      const { participants: splitParticipants, canSplitScreenClosed: isItemSplitValid } =
      ...
      if (!isItemSplitValid) {
        toast.error(t('expense_details.add_expense_details.receipt_items.messages.split_per_item'));
        setSplitScreenOpen(true);
        return;
      }

      // A rounded-away item leaves none.
      const itemParticipants = splitParticipants
        .filter((p) => 0n !== (p.amount ?? 0n))
        .map((p) => ({ userId: p.id, amount: p.amount ?? 0n }));

      if (0 === itemParticipants.length) {
        continue;
      }
  ...
  return (
    <div className="flex flex-col gap-3">
      {/* Sibling of the button, not a child: the synthetic click would bubble back and
          re-enter the handler, and an input inside a button is invalid content. */}
      <input
        ref={inputRef}
        type="file"
        accept="image/*"

  ...
  const scanReceipt = React.useCallback(
    async (fileKey: string) => {
      ...
      try {
        ...
        /*
         * An unreadable image still yields a well-formed response with a zero amount and an
         * empty description. Report that instead of overwriting whatever the user has typed
         * with zeros. This is an expected outcome rather than a failure, so it returns
         * quietly instead of throwing into the catch below.
         */
        if (0n === bigIntValue && !data.description) {

...
/*
 * JSON.parse happily returns null, a number or a bare string, none of which throw. Without
 * this guard the parsers below would property-access a non-object and raise a TypeError.
 */
...
export function parseReceiptItemsResponse(text: string): ReceiptItemsScanResult {
  ...
  try {
    ...
    /* The response body holds merchant, date and amounts — log its size, never its content. */
